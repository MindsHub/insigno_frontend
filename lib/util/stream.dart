// taken from https://github.com/ReactiveX/rxdart/blob/master/lib/src/streams/concat.dart,
...
class ConcatStream<T> extends StreamView<T> {
  ...
  ConcatStream(Iterable<Stream<T>> streams) : super(_buildController(streams).stream);
  ...
  static StreamController<T> _buildController<T>(Iterable<Stream<T>> streams) {
    ...
    controller.onListen = () {
      ...
      void moveNext() {
        ...
        subscription?.cancel();
        subscription =
            iterator.current.listen(controller.add, onError: controller.addError, onDone: moveNext);
      }
...
extension ConcatExtensions<T> on Stream<T> {
  ...
  Stream<T> concatWith(Iterable<Stream<T>> other) {
    ...
    return isBroadcast ? concatStream.asBroadcastStream(onCancel: (s) => s.cancel()) : concatStream;
  }
}
